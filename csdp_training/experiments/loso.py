...
def create_global_split(dataset_filepath: str):
    with h5py.File(dataset_filepath, "r") as hdf5:
        subs = list(hdf5.keys())
    
    split_data = {}

    split_data[dataset_filepath] = {"train": [],
                                    "val": [],
                                    "test": subs}
    
    return split_data

def create_loso_split(dataset_filepath: str,
    ...
    for sub in all_subs:
        rest_of_subs = list(filter(lambda x: x != sub, all_subs))
        split_data = {}

        train, val = train_test_split(rest_of_subs, 
                                      test_size=num_validation_subjects)

        split_data[dataset_filepath] = {"train": train,
                                        "val": val,
                                        "test": [sub]}
...
class LOSO_Experiment:
    ...
    def __create_wrapper(self,
                         split,
                         batch_size):
        train_sampler = Full_Train_Dataset_Sampler(self.dataset_path,
                                                   window_size=35,
                                                   splitdata=split)

        val_sampler = Full_Eval_Dataset_Sampler(self.dataset_path,
                                                split_data=split,
                                                split_type="val")

        test_sampler = Full_Eval_Dataset_Sampler(self.dataset_path,
                                                 split_data=split,
                                                 split_type="test")
